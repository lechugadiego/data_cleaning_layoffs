...
DELETE layoffs_staging
FROM world_layoffs.layoffs_staging
INNER JOIN DELETE_CTE
ON layoffs_staging.company = DELETE_CTE.company
AND layoffs_staging.location = DELETE_CTE.location
AND layoffs_staging.industry = DELETE_CTE.industry
AND layoffs_staging.total_laid_off = DELETE_CTE.total_laid_off
AND layoffs_staging.percentage_laid_off = DELETE_CTE.percentage_laid_off
AND layoffs_staging.date = DELETE_CTE.date
AND layoffs_staging.stage = DELETE_CTE.stage
AND layoffs_staging.country = DELETE_CTE.country
AND layoffs_staging.funds_raised_millions = DELETE_CTE.funds_raised_millions;
...
-- Also noticed Crypto has multiple different variations. We need to standardize that - let's say all to Crypto
SELECT DISTINCT industry
FROM world_layoffs.layoffs_staging2
ORDER BY industry;
...
-- having them null will make it easier for calculations during the EDA phase
